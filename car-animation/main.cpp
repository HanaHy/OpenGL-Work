/*
 *
 * ECS 175 FINAL PROJECT - HANA HYDER
 * Mario Kart + Super Mario Galaxy Theme
 *
 */
//#include<GL/glut.h>
/*#include <OpenGL/gl.h>
...
void load_obj(const char* filename, vector<Pt> &vertices, vector<Face> &faces, vector<Pt> &textCoords)
{
...
{	// Constants for different views
  HELICOPTER,FRONT,SIDE,BACK,RSIDE
...
void ring(float R1,float R2, float r0, float g0, float b0, float rEnd, float gEnd, float bEnd)
{
  float X,Y,Z;
  int  y;
  //int incr = 570;
...
void tree2(float a,float b)
{
  ...
  glColor3f(1,0.2,0);
  glPushMatrix();
		glTranslatef(a,b, -1 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(3))));
		glRotatef(90,1,0,0);
		cylinder(6,25);
  glPopMatrix();
...
void scenery()
{
  ...
  ring(520, 540, 1.0, 0.0, 0.0, 1.0, 0.73, 0.0);
  ring(560, 540, 1.0, 0.95, 0.0, 0.48, 1.0, 0.14);
  ring(580, 560, 0.0, 1.0, 0.83, 0.0, 1.0, 0.0);
  ring(600, 580, 0.0, 0.0, 1.0, 0.0, 1.0, 1.0);
  ring(620, 600, 0.5, 0.0, 1.0, 0.5, 0.24, 0.88);
 int alt = 1; 
  //lumas trees
  for(p=0;p<=360;p+=30)
  {
...
void luma2()
{
  ...
  {
    //glColor3f(rand()/RAND_MAX, 1, 0);
    glColor3f(static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(0.7))), 1.0, 1.0);
    //glColor3f(1, (rand()) / static_cast <float> (RAND_MAX), 0, 1);
    glVertex3f(vertices[faces[i].Pta].x, vertices[faces[i].Pta].y, vertices[faces[i].Pta].z);
    glVertex3f(vertices[faces[i].Ptb].x, vertices[faces[i].Ptb].y, vertices[faces[i].Ptb].z);
    glVertex3f(vertices[faces[i].Ptc].x, vertices[faces[i].Ptc].y, vertices[faces[i].Ptc].z);
  ...
  switch(viewpoint)
  {
    ...
    case BACK:
     ...
     case RSIDE:
      ...
      gluLookAt(20.0,40.0,20,0.0,0.0,2.0,0.0, 0.0,1.0);
      car();
      glPushMatrix();
      glRotatef(angle*RAD_TO_DEG, 0.0,0.0,1.0);
      glTranslatef(-carx,-cary,0);
...
void idle()
{
  ...
  if(start==1)
  {
    ...
    {
      case 1:
      case 0:viewpoint=HELICOPTER; break;
      case 2:
      case 3: viewpoint=RSIDE;break;
      case 6: 
      case 4:
      case 5:viewpoint=SIDE;break;
        
      case 8:
      case 7:viewpoint=FRONT;break;
        
      case 9:
      case 10:viewpoint=BACK;break;
...
void timerFunc(int value)
{
  ...
  glutPostRedisplay();
  //cout << "I REACHED HERE AND COUNT IS " << count << endl;
  count++;
