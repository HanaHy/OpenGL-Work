...
Point **Points; 
float **weights; 
unsigned int knotsInU, knotsInV, pointsInU, 
             pointsInV, degreeU, degreeV, Uorder, 
             Vorder, netSize = 25; // default 25x25 points calculated

float *Uknots; 
float *Vknots;

float MinU() {
  return Uknots[degreeU];
}
float MinV() {
  return Vknots[degreeV];
}
float MaxU() {
  return Vknots[knotsInU-degreeU];
}
float MaxV() {
  return Vknots[knotsInV-degreeV];
}
...
Point findVal(float u, float v)
{
  ...
  Point* temp = new Point[pointsInV]; 
  for(unsigned int i = 0; i < pointsInV; ++i) {
    
    temp[i] = findValU(u, i); 
  }
  
  Point temp2 = findValV(v, temp); 
...
void display (void)
{
  ...
  if(viewpoint == SETPTS)
  {
    if(stptOnce == 0)
    {
      ...
      cout << "Enter pointsInU: "; 
      cin >> pointsInU; 
      cout << "Enter pointsInV: "; 
      cin >> pointsInV; 
      
      
      Points = new Point*[pointsInU]; 
      weights = new float*[pointsInU]; 
      
      for(int i = 0; i < pointsInU; i++)
      {
        Points[i] = new Point[pointsInV]; 
        weights[i] = new float[pointsInV]; 
      }
      ...
      Uknots = new float[Uorder + pointsInU];
      knotsInU = Uorder + pointsInU;
      tmp = Uorder + pointsInU; 
      
      cout << "NOW ENTERING KNOTS IN U DIRECTION-------" << endl; 
      ...
      Vknots = new float[Vorder + pointsInV];
      
      knotsInV = Vorder + pointsInV;
      
      tmp = Vorder + pointsInV; 
      
      cout << "NOW ENTERING KNOTS IN V DIRECTION-------" << endl; 
      ...
      cout << "NOW ENTERING WEIGHTS--------------------" << endl; 
      
      for(int i = 0; i < pointsInU; i++)
      {
        for(int j = 0; j < pointsInV; j++)
        {
          cout << "Entering for weight[" << i << "][" << j << "]: " << endl; 
          cin >> weights[i][j]; 
        }
      }
  ...
  for(i = 0; i < netSize; ++i) {
    ...
    float u = (MaxU() - MinU())*(float)i/(netSize -1) + MinU();
   // float u = ((Vknots[knotsInU - degreeU] - Uknots[degreeU]))*(float)i/(netSize - 1) + Uknots[degreeU];
    ...
    for(j = 0; j < netSize; ++j) {
      ...
      float v = (MaxV() - MinV())*(float)j/(netSize - 1) + MinV();
      //float v = ((Vknots[knotsInV - degreeV] - Vknots[degreeV]))*(float)j/(netSize - 1) + Vknots[degreeV];
...
Point findValU(float t, int row)
{
  Point temp = {0, 0, 0}; 
		...
		for(unsigned int i = 0; i < pointsInU; ++i) {
          
          float Val = coefficients(Uknots, t, i, Uorder); 
          
          if(Val > 0) {
            
            temp.x += Val * Points[row][i].x; 
            temp.y += Val * Points[row][i].y; 
            temp.z += Val * Points[row][i].z; 
          }
        }
...
Point findValV(float t, Point* pnts)
{
  ...
  for(unsigned int i = 0; i < pointsInV; ++i) {
    ...
    float Val = coefficients(Vknots, t, i, Vorder); 
